...
def get_search_plan(user_input):
    ...
    2. **[검색어(keyword) 선정 핵심]** - 공식 명칭보다 **커뮤니티에서 실제로 쓰이는 '은어', '줄임말'**을 최우선으로 선택해. (예: 블루 아카이브 -> 블아/몰루, 리그오브레전드 -> 롤)
       - 특정 사건/이슈에 대해 묻는다면 **'주제어 + 이슈 키워드'** 조합도 고려해. (예: "페이커 부상 반응" -> "페이커 부상" 또는 "대상혁")
